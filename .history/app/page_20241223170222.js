...
export default function Home() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const roundedTemp = Math.round(celsiusTemp); // Round the temperature to nearest integer
      console.log("Rounded Temperature (Celsius):", roundedTemp); // Log the rounded temperature

      // Update the weather data with rounded temperature
      setWeatherData({
        ...response.data,
        main: {
          ...response.data.main,
          temp: roundedTemp, // Show rounded temperature
        },
      });
      setError("");
    } catch (err) {
